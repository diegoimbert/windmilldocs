---
description: How do I suspend a flow and wait for approval or resume events in Windmill?
---
...
When adding a step to a flow, pick `Approval`, and write a new approval script or pick one from [WindmillHub](https://hub.windmill.dev/). This will create a step where the option in tab "Advanced" - "Suspend" is enabled.
...
By default, resume URLs are tied to a specific step. With `flowLevel: true` (TypeScript) or `flow_level=True` (Python), you can generate resume URLs for the parent flow instead. These "pre-approvals" can be consumed by any later suspend step in the same flow.
...
```ts
// TypeScript
const urls = await wmill.getResumeUrls("approver1", true) // flowLevel = true
```

```python
# Python
urls = wmill.get_resume_urls(approver="approver1", flow_level=True)
```

When a flow-level resume is received, the approval is stored at the flow level and matched when the worker checks for resumes at any subsequent suspend step.
...
The user who triggered the flow will not be allowed to approve it. This restriction is enforced both on the approval page and on the flow resume API endpoint, preventing flow owners from bypassing approval requirements programmatically.
