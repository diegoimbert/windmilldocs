...
Windmill employs lightweight telemetry to automatically track and report the usage of memory and seats for your subscription. The telemetry endpoint is `stats.windmill.dev`: [Enterprise Edition](/pricing) instances are required to send telemetry tied to their license key to that endpoint so we can verify usage stays within the subscription terms. Telemetry cannot be fully disabled but can be reduced to a minimal mode (see [Instance settings - Telemetry](../../advanced/18_instance_settings/index.mdx#telemetry)).
