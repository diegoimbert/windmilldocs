...
### Workspace forks and billing

On [Windmill Cloud](https://app.windmill.dev/), [workspace forks](../../advanced/20_workspace_forks/index.mdx) are available for workspaces on a paid (Team or Enterprise) plan. A fork does not carry its own subscription: it inherits the plan and usage limits of its parent (root) workspace, and its executions are metered into the root workspace's usage so they draw from the same pool and count toward the root's usage-based bill. The number of forks a workspace family can have is capped at 5 per paid (developer) seat of the root workspace. Free-tier workspaces cannot create forks. See [Forks on Windmill Cloud](../../advanced/20_workspace_forks/index.mdx#forks-on-windmill-cloud) for details.
...
- Seats: number of users (1 developer, or 2 operators) who are active (from logging in to running or deploying a script) on the platform in the last 30 days, according to the [audit logs](../../core_concepts/14_audit_logs/index.mdx). For [external JWT](../../advanced/16_external_auth_with_jwt/index.mdx) authentication, a unique JWT user is counted per (username/email, scope, instance) tuple used in the last 30 days - so the same email with two different scopes, or on two different instances, counts as two JWT users. Each unique JWT user counts as 1/2 seat (same as an operator). Regular and operator user counts are across all instances (dev, prod) but Windmill only counts once the same user; external JWT users are counted per instance.
...
- A native worker counts as 1 compute unit. It runs 8 native subworkers, which go 8 by 8. Native workers are workers within the native worker group. This group is pre-configured to listen to native jobs tags (query languages). Those jobs are executed under a special mode with native subworkers for increased throughput. You can set the number of native workers to 0.
...
Calculation: 2 workers × 1 CU each + 1 native worker with 8 native subworkers = 3 CUs
...
services:
  ...
  windmill_worker_native:
    deploy:
      replicas: 1  # 1 native worker with 8 native subworkers = 1 CU
      resources:
        limits:
          memory: 2048M
    environment:
...
Calculation: 3 workers × 1 CU each + 1 native worker with 8 native subworkers = 8 CUs
...
Calculation: 2 workers × 1 CU each + 4 workers × 3 CUs each + 1 native worker with 8 native subworkers = 16 CUs
...
services:
  ...
  windmill_worker_native:
    deploy:
      replicas: 1  # 1 native worker with 8 native subworkers (can't be changed) = 1 CU
      resources:
        limits:
          memory: 2048M
    environment:
...
Calculation: 2 workers × 1 CU each + 4 workers × 2 CUs each + 1 native worker with 8 native subworkers = 12 CUs
...
- `extraEnv`: Environment variables for the native workers:
  - `NATIVE_MODE`: Automatically spawns 8 native subworkers and restricts the worker to native jobs + flow handling
...
When enabled, instances send unhashed email addresses alongside usage telemetry — letting you see the actual users counted as developers and operators across all your instances (deduplicated) — and the [external JWT users](../../core_concepts/16_roles_and_permissions/index.mdx) (counted per username/email + scope + instance) seen in the last 30 days, with their owner, workspace, label, and scopes. This setting auto-disables 24 hours after being turned on.
...
Our [Trust center](https://trust.windmill.dev) and SOC 2 compliance cover the Enterprise Edition only. The Community Edition uses different code paths for security and is not covered by these certifications.

## Usage checks

Windmill employs lightweight telemetry to automatically track and report the usage of memory and seats for your subscription. The telemetry endpoint is `stats.windmill.dev`: [Enterprise Edition](/pricing) instances are required to send telemetry tied to their license key to that endpoint so we can verify usage stays within the subscription terms. Telemetry cannot be fully disabled but can be reduced to a minimal mode (see [Instance settings - Telemetry](../../advanced/18_instance_settings/index.mdx#telemetry)). If you have fully air-gapped requirements, [contact us](../19_enterprise_onboarding/index.mdx#air-gapped-environments) to arrange an alternative.

For [air-gapped Enterprise Edition](../19_enterprise_onboarding/index.mdx#air-gapped-environments) instances that cannot reach `stats.windmill.dev`, the same telemetry can be exported as a signed JSON file from [Instance settings - Telemetry](../../advanced/18_instance_settings/index.mdx#telemetry) ("Download usage") and uploaded to the [Customer Portal](#windmill-customer-portal). The signature lets Windmill verify the file has not been tampered with, and the file can be inspected before being shared.
