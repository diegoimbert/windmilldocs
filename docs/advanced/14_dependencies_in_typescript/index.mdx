---
description: How do I manage TypeScript dependencies in Windmill using Bun and Deno?
---
...
You may also use absolute imports. The recommended form uses the `$f/` and `$u/` aliases, which resolve the same way on the Windmill worker and locally - both for editor type hints and when running scripts, in Bun and Deno:
...
`$f/` points to folder scripts (`f/<foldername>/...`) and `$u/` to user-space scripts (`u/<username>/...`). On [local development](../4_local_development/index.mdx), `wmill init` (or `wmill refresh tsconfig`) generates the configuration that resolves these aliases: a wmill-managed `tsconfig.wmill.json` holding the recommended `compilerOptions`, and a `tsconfig.json` that extends it. Deno projects get an `import_map.wmill.json` wired through `deno.json` instead.

The older `/f/` and `/u/` absolute form still works on the worker. Locally, adding this mapping to tsconfig.json lets Bun resolve it too:
...
Bun applies tsconfig `paths`, so this resolves at runtime, but the leading `/` makes the import non-portable: Deno resolves through import maps rather than tsconfig `paths`, and other tools treat `/f/...` as a filesystem-root path. Prefer `$f/` / `$u/`, a bare specifier that resolves consistently through the generated config.
