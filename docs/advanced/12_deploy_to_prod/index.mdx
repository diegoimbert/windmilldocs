---
description: How do I deploy to production in Windmill? Set up staging/prod environments with Git-based deployment and workspace forks.
---
...
:::tip Canonical deployment setups

If you're looking for opinionated, step-by-step guidance, see [Canonical deployment setups](../23_canonical_deployment_setups/index.mdx) for three recommended deployment models ranging from UI-only to Git-based workflows.
...
Deployments in Windmill are commonly done from the same workspace using the [Draft and deploy](../../core_concepts/0_draft_and_deploy/index.mdx) workflow. Drafts are user-scoped and automatically saved as you edit, then deployed when ready.
...
For a long-lived dev/prod pair instead of throwaway branches, use a [dev workspace](../26_dev_workspaces/index.mdx): a persistent fork that is the standing editable environment, paired with a prod workspace that can be locked so changes only reach it through promotion.
