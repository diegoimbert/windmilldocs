...
We already have pre-set integrations (or resource types), the list is available on our [Hub](https://hub.windmill.dev/resources) (200+ resource types), [using Windmill](../intro.mdx) (most up-to-date version), and at the end of this article.
...
- A [resource type](../core_concepts/3_resources_and_types/index.mdx#create-a-resource-type) defines the JSON schema (host, credentials, options) for that system. Pick one of the 200+ resource types on the [Hub](https://hub.windmill.dev/resources), or define your own.
