---
description: What are resources in Windmill? JSON objects that store connections to databases, APIs and third-party services.
---
...
This is how Windmill connects to any third-party system: pick one of the 200+ resource types available on the [Hub](https://hub.windmill.dev/resource_types), or [define your own](#create-a-resource-type) for anything internal or not yet covered. There are no vendor-branded connectors - the resource type is the integration.

![Recap Resources and Types](./recap_resources_and_types.png 'Recap Resources and Types')

While resources store configuration and credentials for connections, [Assets](../52_assets/index.mdx) track data flows and visualize datasets. Assets automatically detect when you reference [S3 objects](../38_object_storage_in_windmill/index.mdx#workspace-object-storage) (`s3://storage/path/to/file.csv`) or resources (`res://path/to/resource`) directly in your code, creating a visual representation of your data pipeline. This helps you understand how data moves through your workflows and identify dependencies between different data sources.
...
Resource values support two other placeholders, resolved at the moment the resource is fetched:

- `$res:<RESOURCE_PATH>` — embeds another Resource's value. Like `$var:`, it is resolved with the caller's permissions, so the fetch fails if the caller cannot read the referenced resource.
- `$WM_*` — a [contextual variable](../47_environment_variables/index.mdx#contextual-variables) (for example `$WM_TOKEN`, `$WM_JOB_ID`, `$WM_WORKSPACE`), resolved to the running job's context when the resource is fetched as part of a job. Outside of a job context it is left unchanged.

:::info Only these placeholders are interpolated

Inside a Resource value, only `$var:`, `$res:` and `$WM_*` are substituted. Any other
`$`-prefixed string — including [custom workspace environment variables](../47_environment_variables/index.mdx#custom-environment-variables) —
is returned as-is and is **not** interpolated. (Custom environment variables are still
injected into Scripts and workers; they are simply not resolved inside Resource values.)
...
Windmill comes preloaded with some common Resource types, such as databases, apps, SMTP, etc. You can see the full list on [Windmill Hub](https://hub.windmill.dev/resource_types).
...
You can contribute to the [Windmill Hub](https://hub.windmill.dev/) by sharing your Resource Type. To do so, add a Resource Type on the [Resources section](https://hub.windmill.dev/resource_types) of the Hub.
...
**Important note:** Don't store large content in flow user state. For files, store them on [S3](../38_object_storage_in_windmill/index.mdx) and store the path in flow user state.
...
Resources of type `json_schema` allow you to store reusable [JSON schemas](../13_json_schema_and_parsing/index.mdx) that define the structure and validation rules for object parameters. 
They contain a single `schema` field that is a JSON schema object.
These schemas can be referenced as templates when configuring object-type parameters in flows.
