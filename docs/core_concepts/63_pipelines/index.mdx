...
This minimal example uses TypeScript and a plain S3 file to keep the wiring model in focus. In practice, the canonical step is a DuckDB query that [materializes](#materialization) into a managed [DuckLake](../11_persistent_storage/ducklake.mdx) table - that is what unlocks idempotent writes, time-travel, column lineage, data tests and backfill (see [the canonical stack](#the-canonical-stack-duckdb--ducklake--materialize)).
