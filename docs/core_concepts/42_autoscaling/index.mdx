---
description: How do I configure autoscaling to adjust worker count based on workload in Windmill?
---
...
When scaling in, the integration annotates the worker pods with [`controller.kubernetes.io/pod-deletion-cost`](https://kubernetes.io/docs/concepts/workloads/controllers/replicaset/#pod-deletion-cost) just before lowering the replica count, so the ReplicaSet controller deletes idle pods first instead of picking victims blindly (which could leave a worker mid-job terminating while idle pods stay alive):

- Idle pods get a cost of 0.
- Pods currently running a job get a cost of `10 + age of their oldest running job in seconds` (capped at 1 day), so workers on long-running jobs are protected hardest.
...
The health check also probes the optional pods permissions used for [pod-deletion-cost annotations](#scale-in-prefers-idle-workers) and logs a non-fatal warning if they are missing: autoscaling stays fully functional, it just loses the idle-pods-first preference on scale-in.
