...
To override any of these defaults for a route, set the corresponding header on the response returned by your runnable using the [`wm_headers`](../4_webhooks/index.mdx#custom-response-headers) mechanism (or its longer alias `windmill_headers`). Only headers that are **not** already present in the response are filled in with the defaults above, so you can restrict origins or allowed headers on a per-route basis:
