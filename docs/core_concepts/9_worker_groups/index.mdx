---
description: What are worker groups in Windmill? Assign jobs to specific workers, manage concurrency and resource allocation.
---
...
- `duckdb`: The default worker group for [DuckDB](../../getting_started/0_scripts_quickstart/5_sql_quickstart/index.mdx#duckdb-1) scripts.
...
This group is pre-configured to listen to native jobs tags. Those jobs are executed under a special mode with native subworkers for increased throughput.

Setting the `NATIVE_MODE=true` [environment variable](../47_environment_variables/index.mdx) on any worker group automatically spawns 8 native subworkers (which go 8 by 8) and restricts the worker to only accept native jobs (bunnative, nativets, postgresql, mysql, mssql, graphql, snowflake, bigquery, oracledb — no dependency jobs) and flow handling. The "native" dedicated worker group always has this mode enabled. It can also be toggled from the UI on any worker group.
...
- `oracledb`: The default worker group for [OracleDB](../../getting_started/0_scripts_quickstart/5_sql_quickstart/index.mdx) scripts.
...
**Note**: In the Community Edition, the worker group management UI allows creating worker group configs and editing `worker_tags`. Advanced features such as [dedicated workers](../25_dedicated_workers/index.mdx), custom environment variables, [init/periodic scripts](#worker-scripts), autoscaling, and priority tags require [Cloud plans & Self-Hosted Enterprise Edition](/pricing). You can also pass tags directly as an env variable:
...
:::note Workspace-specific tags and workspace forks

When workspace-specific default tags are enabled, [workspace forks](../../advanced/20_workspace_forks/index.mdx#workspace-specific-tags-and-forks) automatically resolve to their parent workspace's tag so fork jobs are routed to the same workers as the parent. An optional `-fork` suffix can be enabled in instance settings to route fork jobs to dedicated workers instead.
...
Workers can be commonly managed based on the group they are in, from the UI. In the Community Edition, you can create worker group configs and edit their tags. Advanced features like [dedicated workers](../25_dedicated_workers/index.mdx), custom environment variables, [init/periodic scripts](#worker-scripts), autoscaling, and priority tags require [Cloud plans & Self-Hosted Enterprise Edition](/pricing).
...
Configure Python-specific settings at the worker group level.

![Python runtime settings](./python_runtime_settings.png 'Python runtime settings')

- **Additional Python paths**: extra directories to add to `sys.path` at runtime, making any Python modules in those directories importable by scripts.
- **PIP local dependencies**: regex patterns for packages that the [dependency resolver](../../advanced/15_dependencies_in_python/index.mdx) should skip during lockfile generation. Use this for packages pre-installed on the worker (e.g. via [global site-packages](../../advanced/8_preinstall_binaries/index.mdx#global-site-packages) or a custom Docker image) so that their `import` lines don't trigger PyPI resolution. Can also be set via the `PIP_LOCAL_DEPENDENCIES` environment variable (comma-separated).
...
<div className="grid grid-cols-2 gap-6 mb-4">
	...
	/>
	<DocCard
		title="Global site-packages"
		description="Pre-install Python packages on workers and skip dependency resolution."
...
Workers in a group can also be restarted via the API endpoint `POST /api/settings/restart_worker_group/{worker_group}`. This is a global endpoint (not workspace-scoped) that requires devops permissions. The restart is staggered with DB coordination to avoid downtime.
...
Set the `QUIET` [environment variable](../47_environment_variables/index.mdx) to `true` or `1` on workers to reduce log verbosity. When enabled:

- Verbose per-job logs (fetched job, started handling, job finished) are suppressed.
- Periodic "still running" and memory snapshot logs are emitted 10x less frequently.

This is useful in production environments where per-job log entries create excessive noise. A "Quiet mode enabled" message is added to job logs for visibility.
