---
description: What are agent workers and how do I run Windmill jobs remotely behind firewalls or in untrusted environments?
---
...
In terms of [billing](../../misc/7_plans_details/index.mdx), agent workers are counted as 1 [Compute Unit](/pricing) per agent worker on self-hosted EE (if not used for compute) and 0.5 Compute Unit on cloud EE (if not used for compute).
...
For the narrow case where you cannot run any process in the target environment and can only reach a jump node over SSH, bash scripts can be rerouted to a remote host with the [`#ssh` directive](../../advanced/25_ssh_execution/index.mdx). Agent workers remain the recommended option whenever they are possible.
