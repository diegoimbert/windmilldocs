---
description: What are the storage options in Windmill? S3/R2/MinIO object storage, Postgres, DuckDB, data tables and key-value stores.
---
...
There are several kinds of persistent storage in Windmill:

1. [Data tables](#data-tables) for out-of-the-box relational data storage.

2. [Ducklakes](#ducklake) for data lakehouse storage.

3. [Small data](#states-resources--shared-directory) that is relevant in between script/flow execution and can be persisted on Windmill itself.
...
7. [Volumes](#volumes) for persistent file storage attached to scripts, synced to object storage between runs.
...
Windmill has internal methods to persist data between executions of jobs: [states](../3_resources_and_types/index.mdx#states) and [resources](../3_resources_and_types/index.mdx) to store small transient JSON, [custom flow states](../3_resources_and_types/index.mdx#custom-flow-states) to share data across the steps of a flow, and the shared directory to pass files between steps running on the same worker.

These are convenient for small or transient payloads but are not recommended as a general-purpose persistence layer: states and resources are meant for small JSON, and the shared directory is ephemeral to a single flow execution. For larger or long-lived data, prefer an external database or object storage.

<div className="grid grid-cols-2 gap-6 mb-4">
	<DocCard
		title="States, resources & shared directory"
		description="Use states, resources, custom flow states, and the shared directory to persist small or transient data between jobs and across flow steps."
		href="/docs/core_concepts/persistent_storage/states_resources_shared_directory"
	/>
</div>
...
[Volumes](../56_volumes/index.mdx) provide persistent file storage that can be attached to scripts via code annotations. Files written to a volume during a job run are synced back to object storage and restored on the next run, enabling stateful workflows like caching, model storage, or agent memory.

<div className="grid grid-cols-2 gap-6 mb-4">
	<DocCard
		title="Volumes"
		description="Persistent file storage attached to scripts via annotations."
