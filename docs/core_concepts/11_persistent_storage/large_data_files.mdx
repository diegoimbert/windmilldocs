---
description: How do I store large files and binary data using S3, R2, MinIO, Azure Blob, or Google Cloud Storage in Windmill?
---
...
We recommend using the native DuckDB scripts integration, which uses the Windmill [S3 proxy](../object_storage_in_windmill#s3-proxy) with no additional configuration needed.
When using DuckDB from Python, the S3 resource details need to be visible to the user, or [set to public in the workspace settings](../38_object_storage_in_windmill/index.mdx#workspace-object-storage).
