---
description: How do I handle errors in Windmill? Use error handlers, retries, custom error messages and recovery strategies.
---
...
## Tag a successful run as a failure

A script or flow can return a top-level `wm_failure` field (string) to retag the run as a failure even though the runtime did not raise an error. The string becomes the error message, and the run takes the standard failure path: the run is marked as failed on the [Runs page](../5_monitor_past_and_future_runs/index.mdx), configured error handlers fire, and the stored result is wrapped as `{ "error": { "name": "ManualFailure", "message": <wm_failure> }, ... }`.

```ts
export async function main() {
  return {
    wm_failure: "Upstream API returned 500",
    foo: "bar"
  }
}
```

This applies to any job, regardless of how it was triggered (manual run, schedule, flow step, webhook). On [synchronous webhook endpoints](../4_webhooks/index.mdx#synchronous), sibling fields such as `wm_status_code`, `wm_content_type`, and `wm_headers` are still honored, so a manually-tagged failure can also set a custom HTTP status code or response headers.
...
### Skip error handler for user-scoped scripts and flows

A workspace setting allows disabling the error handler for scripts and flows under `u/` (user) paths. When enabled, errors in user-scoped runnables will not trigger the workspace error handler. This is useful to avoid noise from personal development scripts.

## Workspace success handler

Configure a script to run automatically when jobs complete successfully in the workspace. This is the counterpart to the [workspace error handler](#workspace-error-handler) — while the error handler runs on failures, the success handler runs on successful completions.

Workspace success handler is an [Enterprise Edition](/pricing) feature.

Configure workspace success handlers from **Workspace Settings > Error Handler** tab (in the success handler section).

**Parameters passed to success handlers:**

- `path`: The path of the script or flow that succeeded
- `email`: The email of the user who ran the script or flow
- `result`: The result of the successful job
- `job_id`: The job id
- `is_flow`: Whether the job is a flow
- `workspace_id`: The workspace id
- `started_at`: When the job started

**Success handler template:**
...
export async function main(
	...
	const run_type = is_flow ? 'flow' : 'script';
	console.log(
		`Success: ${run_type} ${path} run by ${email} completed in workspace ${workspace_id}`
	);
	console.log(`Job ${job_id} started at ${started_at}`);
	console.log('Result:', JSON.stringify(result));

	// Add your custom success handling logic here
	// Examples: send metrics, update dashboards, notify on completion

	return { handled: true, workspace_id, job_id };
}
```

The success handler is executed by the automatically created group `g/success_handler`. If your handler requires variables or resources, add them to this group.

:::info Cache propagation
Success handler settings are cached for 60 seconds. After updating the configuration, it may take up to 60 seconds for the change to take effect.
:::
