...
ETLs are easily implemented in Windmill using its integration with [DuckDB](../../integrations/duckdb.md) to work with tabular data. You don't need to manually interact with the S3 bucket: DuckDB reads and writes datasets to S3 natively and efficiently.
...
<TabItem value="duckdb" label="DuckDB (SQL)" attributes={{className: "text-xs p-4 !mt-0 !ml-0"}}>
...
DuckDB needs to be configured to access S3 within the Windmill script. The job needs to access the S3 resources, which either need to be accessible to the user running the job, or the S3 resource needs to be [set as public in the workspace settings](../38_object_storage_in_windmill/index.mdx#workspace-object-storage).
...
If you use Python, the Windmill SDK exposes a connection helper so you don't hand-write S3 credentials and `SET s3_...` blocks in every script. You pull the settings from the workspace storage (or a designated resource) instead:
...
With Windmill providing the connection boilerplate and DuckDB reading and writing from/to S3 natively, your scripts stay concise and focused on processing the data. See [Object storage in Windmill](../38_object_storage_in_windmill/index.mdx#workspace-object-storage) for the full connection-settings reference.

In the end, a canonical pipeline step in Windmill is a DuckDB script that reads from S3, transforms, and writes the result back for the next step to pick up:
