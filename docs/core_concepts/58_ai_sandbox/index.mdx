...
- `sandbox` - runs the [job](../20_jobs/index.mdx) inside [nsjail](../../advanced/security_isolation/index.mdx#nsjail-sandboxing), isolating the agent's filesystem and processes from the [worker](../9_worker_groups/index.mdx).
- `volume: <name> <path>` - mounts a persistent [volume](../56_volumes/index.mdx) so the agent can read and write files that survive across job runs (session state, memory, generated artifacts, etc.). Files are synced to [object storage](../38_object_storage_in_windmill/index.mdx).

This pattern works for any [language](../../getting_started/0_scripts_quickstart/index.mdx) supported by Windmill. The agent itself can be invoked via an SDK, a CLI subprocess, or an HTTP API - the sandbox and volume annotations are independent of how the agent runs. You can use AI sandboxes in standalone scripts or as steps within [flows](../../flows/1_flow_editor.mdx).
...
Windmill includes a built-in template for [Claude Code](https://docs.anthropic.com/en/docs/claude-code) using the [`@anthropic-ai/claude-agent-sdk`](https://www.npmjs.com/package/@anthropic-ai/claude-agent-sdk). It demonstrates the sandbox + volume pattern with session persistence.
...
- [Workers](../../core_concepts/9_worker_groups/index.mdx) with [nsjail](https://github.com/google/nsjail) available (included in all standard Windmill [Docker images](../../advanced/1_self_host/index.mdx)).
- For the Claude Code template: an [Anthropic](https://console.anthropic.com/) [resource](../3_resources_and_types/index.mdx).

## Passing credentials to the sandbox

Script inputs are TypeScript (or Python) variables - they are **not** automatically available as [environment variables](../../script_editor/custom_environment_variables.mdx) inside the sandbox. This matters because AI agents like Claude Code spawn shell processes (e.g. `git push`, `curl`) that look for credentials in environment variables, not in your script variables.
