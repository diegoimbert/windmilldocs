...
In Windmill, each step is [code](/docs/getting_started/scripts_quickstart) with a main function in TypeScript, Python, Go, PHP, Bash or raw queries to databases.
...
From Windmill, pick `+ Flow`, and either import YAML or JSON from the [Hub](https://hub.windmill.dev/flows/45/) or start building manually.
...
<details>
  <summary>Code: Extract content from GitHub (TypeScript)</summary>
...
<details>
  <summary>Code: Store the embeddings on Supabase</summary>
...
1. [Verify and defer the request](#verifying-the-discord-request-and-defer) from Discord.
1. [Extract the question](#extract-the-question-from-the-discord-request) from the Discord request.
1. [Create the answer](#create-the-answer-using-the-question-the-embeddings-and-openai) using the question and the embeddings and OpenAI.
1. [Create an approval step](#create-the-approval-step-to-validate-the-answer-by-a-human) to validate the answer by a human.
1. If validated, [send the answer publicly](#send-the-answer-back-to-discord).

### Verifying the Discord request and Defer

We need to verify the request from Discord and defer it. This is a requirement from Discord.
Discord gives us the following parameters as query parameters:
...
We can pipe these parameters to the script or flow inputs using the `include_header` query parameter See [Building the URL for the Discord Interaction endpoint](#building-the-url-for-the-discord-interaction-endpoint).
...
<details>
  <summary>Code: Verifying the Discord request and Defer</summary>
...
<details>
  <summary>Code: Extract the question from the Discord request</summary>
...
<details>
	<summary>Code: Create the answer using the question and the embeddings and OpenAI</summary>
...
<details>
	<summary>Code: Create the approval step to validate the answer by a human</summary>
...
<details>
	<summary>Code: Send the answer back to Discord</summary>
...
1. Go into the Webhooks section of the "[Discord flow](#verifying-the-discord-request-and-defer)" flow's `Detail` page.
...
![Early return](./early_return.png 'Early return')
...
<details>
	<summary>Code: Create the Slack endpoint</summary>
