...
- **Agent**: connects to a remote Windmill server over HTTPS, with no direct database access. Ideal for workers behind firewalls. See [agent workers](/docs/core_concepts/agent_workers) for how to generate tokens
...
Configure your Azure Artifacts feed URL and PAT in [instance settings](/docs/advanced/imports#private-repository-azure-artifacts-feed), then use `Import-Module` as usual. Windmill checks the private feed first and falls back to PowerShell Gallery.
...
Import-Module CompanyInternalTools
...
Windmill supports SSO via [Azure AD OAuth](/docs/misc/setup_oauth). The same identity can authenticate against MSSQL using `aad_token`, so one login covers both platform access and database access.
